...
class PdfRenderContext implements ChartRenderContext {
  ...
  fillRect = (x: number, y: number, width: number, height: number) => {
    ...
    this.pdf.rect(this.offsetX + x, this.offsetY + y, width, height, 'F');
  };
  ...
  stroke = () => {
    ...
    if (this.circle) {
      this.pdf.circle(
        this.offsetX + this.circle.x,
        this.offsetY + this.circle.y,
        this.circle.r,
        'S'
      );
      return;
    ...
    this.segments.forEach((segment) => {
      this.pdf.line(
        this.offsetX + segment.x1,
        this.offsetY + segment.y1,
        this.offsetX + segment.x2,
        this.offsetY + segment.y2
      );
    });
  };
  ...
  fillText = (text: string, x: number, y: number) => {
    ...
    this.pdf.text(text, this.offsetX + x, this.offsetY + y, { align, baseline });
  };
...
export const ExportDialog = ({
  ...
  const renderPdfPages = async (
    ...
    const chartContext = new PdfRenderContext(pdf, chartXOffset, chartYOffset, chartFontName);
    drawPsychrometricChart(
      chartContext,
      drawChartWidthMm,
      drawChartHeightMm,
