...
export const ExportDialog = ({
  ...
  const createChartImageDataUrl = (drawChartWidthMm: number, drawChartHeightMm: number) => {
    const chartRenderWidth = Math.round(mmToPx(drawChartWidthMm));
    const chartRenderHeight = Math.round(mmToPx(drawChartHeightMm));
    const chartRenderCanvas = document.createElement('canvas');
    chartRenderCanvas.width = chartRenderWidth;
    chartRenderCanvas.height = chartRenderHeight;
    renderPsychrometricChart({
    ...
    return chartRenderCanvas.toDataURL('image/png');
  };

  const setPdfFont = (pdf: jsPDF, sizeMm: number, style: 'normal' | 'bold' = 'normal') => {
  ...
  const drawPdfPage1 = (
    ...
    statePointOverflow: StatePoint[],
    processOverflow: Process[]
  ) => {
    const marginMm = 8;
    const headerHeightMm = 14;
    const chartTopMm = marginMm + headerHeightMm + 2;
    ...
    setPdfFont(pdf, 2.2, 'bold');
    pdf.setTextColor(55, 65, 81);
    pdf.text('設計条件', conditionsX, headerY + 2);

    setPdfFont(pdf, 1.8, 'normal');
    ...
    setPdfFont(pdf, 1.7, 'bold');
    pdf.text('外気条件', col1X, headerY + 4.5);
    setPdfFont(pdf, 1.6, 'normal');
    pdf.text(
      `夏: ${formatNumber(designConditions.outdoor.summer.dryBulbTemp)}°C / ${formatNumber(designConditions.outdoor.summer.relativeHumidity, 0)}%`,
      col1X,
    ...
    const chartXOffsetMm = marginMm + (chartWidthMm - drawChartWidthMm) / 2;
    pdf.setDrawColor(229, 231, 235);
    pdf.rect(marginMm, chartY, chartWidthMm, chartHeightMm);

    const chartDataUrl = createChartImageDataUrl(drawChartWidthMm, drawChartHeightMm);
    pdf.addImage(
      chartDataUrl,
      'PNG',
      chartXOffsetMm,
    ...
    const halfWidthMm = contentWidthMm / 2 - 2;

    let statePointY = bottomY;
    pdf.setTextColor(17, 24, 39);
    setPdfFont(pdf, 2.8, 'bold');
    ...
    filteredStatePoints.forEach((point, index) => {
      ...
      setPdfFont(pdf, 1.9, 'bold');
      pdf.text(label, marginMm + 3, statePointY + 2.3, { align: 'center' });

      pdf.setTextColor(17, 24, 39);
      setPdfFont(pdf, 2.1, 'bold');
      pdf.text(point.name, marginMm + 7, statePointY + 2.2);
      ...
      const propTextLine1 = `温度: ${formatNumber(point.dryBulbTemp)}°C | RH: ${formatNumber(point.relativeHumidity, 0)}% | 絶対湿度: ${formatNumber(point.humidity, 4)} kg/kg'`;
      const propTextLine2 = `エンタルピー: ${formatNumber(point.enthalpy)} kJ/kg' | 風量: ${airflowText} | 季節: ${seasonLabel(point.season)}`;
      pdf.text(propTextLine1, marginMm + 7, statePointY + 4.8);
      pdf.text(propTextLine2, marginMm + 7, statePointY + 7);

      statePointY += statePointCardHeight;
    });

    const processX = marginMm + halfWidthMm + 4;
    let processY = bottomY;
    pdf.setTextColor(17, 24, 39);
    if (filteredProcesses.length > 0) {
      setPdfFont(pdf, 2.8, 'bold');
    ...
    filteredProcesses.forEach((process) => {
      ...
      pdf.setTextColor(17, 24, 39);
      setPdfFont(pdf, 2.1, 'bold');
      pdf.text(process.name, processX + 2.5, processY + 2.4);

      pdf.setTextColor(107, 114, 128);
      setPdfFont(pdf, 1.7, 'normal');
      pdf.text(`${fromLabel} → ${toLabel}`, processX + 2.5, processY + 4.4);

      setPdfFont(pdf, 1.6, 'normal');
      detailLines.forEach((line, lineIndex) => {
        pdf.text(line, processX + 2.5, processY + 6.4 + lineIndex * processLineHeight);
      });
  ...
  const drawPdfPage2 = (pdf: jsPDF, statePointOverflow: StatePoint[], processOverflow: Process[]) => {
    const marginMm = 8;
    pdf.setFillColor(255, 255, 255);
    pdf.rect(0, 0, A4_WIDTH_MM, A4_HEIGHT_MM, 'F');

    pdf.setTextColor(17, 24, 39);
    setPdfFont(pdf, 3.2, 'bold');
    ...
    if (statePointOverflow.length > 0) {
      ...
      statePointOverflow.forEach((point) => {
        ...
        pdf.setTextColor(255, 255, 255);
        setPdfFont(pdf, 1.9, 'bold');
        pdf.text(label, marginMm + 3, currentY + 2.3, { align: 'center' });

        pdf.setTextColor(17, 24, 39);
        setPdfFont(pdf, 2.1, 'bold');
        pdf.text(point.name, marginMm + 7, currentY + 2.2);
        ...
        const propTextLine1 = `温度: ${formatNumber(point.dryBulbTemp)}°C | RH: ${formatNumber(point.relativeHumidity, 0)}% | 絶対湿度: ${formatNumber(point.humidity, 4)} kg/kg'`;
        const propTextLine2 = `エンタルピー: ${formatNumber(point.enthalpy)} kJ/kg' | 風量: ${airflowText} | 季節: ${seasonLabel(point.season)}`;
        pdf.text(propTextLine1, marginMm + 7, currentY + 4.8);
        pdf.text(propTextLine2, marginMm + 7, currentY + 7);
    ...
    if (processOverflow.length > 0) {
      ...
      processOverflow.forEach((process) => {
        ...
        pdf.setTextColor(17, 24, 39);
        setPdfFont(pdf, 2.1, 'bold');
        pdf.text(process.name, marginMm + 2.5, currentY + 2.4);

        pdf.setTextColor(107, 114, 128);
        setPdfFont(pdf, 1.7, 'normal');
        pdf.text(`${fromLabel} → ${toLabel}`, marginMm + 2.5, currentY + 4.4);

        setPdfFont(pdf, 1.6, 'normal');
        detailLines.forEach((line, lineIndex) => {
          pdf.text(line, marginMm + 2.5, currentY + 6.4 + lineIndex * processLineHeight);
        });
